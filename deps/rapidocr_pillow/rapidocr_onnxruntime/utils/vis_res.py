...
from rapidocr_onnxruntime._pillow_cv import draw_polyline, draw_text
...
class VisRes:
    ...
    def draw_dt_boxes(self, img_content: InputType, dt_boxes: np.ndarray) -> np.ndarray:
        ...
        for idx, box in enumerate(dt_boxes):
            ...
            points = np.array(box)
            img = draw_polyline(img, points, color=color, thickness=1)

            start_point = round(points[0][0]), round(points[0][1])
            img = draw_text(img, f"{idx}", start_point, color)
        return img
